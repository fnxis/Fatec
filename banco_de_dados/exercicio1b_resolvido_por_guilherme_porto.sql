...
SELECT categoria, modelo, cor 
FROM veiculos 
WHERE diaria BETWEEN 100 AND 200;
...
FROM veiculos 
WHERE categoria = 'suv' 
AND diaria BETWEEN 100 AND 200 
AND quilometragem < 20000;
...
FROM veiculos 
WHERE diaria BETWEEN 100 AND 200 
AND (categoria = 'básico' OR categoria = 'suv');
...
SELECT *
FROM veiculos 
WHERE tipo_combustivel NOT IN ('gasolina', 'diesel','flex');
...
SELECT AVG(quilometragem) AS quilometragem_media
FROM veiculos 
...
WHERE quilometragem < (
    ...
    FROM veiculos t2 
    WHERE t2.categoria = t1.categoria
);
